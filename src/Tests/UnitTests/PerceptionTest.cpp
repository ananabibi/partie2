...
{
public:
    OrientableBuffalo(Vec2d const& position,
                Vec2d const& heading)
        :Buffalo(position)
    {
        setHeading(heading);
    ...
    double getViewDistance() const override {return 300;}
    double getViewRange() const override {return 60 * DEG_TO_RAD;}
};
...
SCENARIO("Animal interacts with target", "[Animal]")
{
	...
	OrientableBuffalo animal(initialPosition, initialOrientation);
   ...
   GIVEN("A target at the same position as the animal")
   {
       ...
       {
           CHECK(animal.isPerceptible(target));
       }
   }
   ...
   GIVEN("A target in the view range of the animal but too distant")
   {
       ...
       {
           CHECK_FALSE(animal.isPerceptible(target));
       }
   }
