...
void Animal::update(sf::Time dt) {
    ...
    if (eauPercue == nullptr) move(dt);
    else move(computeForce(eauPercue->getCenter()), dt);

    mCompteurChangementDirection += dt;
}

enum Deceleration {DECEL_NULLE, DECEL_RAPIDE, DECEL_NORMALE, DECEL_LENTE};
...
Vec2d Animal::computeForce(Vec2d cible) const { // todo modulariser
    ...
    return vTarget - mVitesse; // todo surement changer mVitesse
}
...
void Animal::move(sf::Time dt) {
    ...
    if (mCompteurChangementDirection > temps_entre_deux_rotations) {
        rotate();
        mCompteurChangementDirection = sf::Time::Zero;
    }
...
bool Animal :: isPerceptible(Vec2d const& position, double offset) const {
    Vec2d d = getPosition() - position;

    double squaredViewDistance = (getViewDistance() + offset) * (getViewDistance() + offset);
    bool isCloseEnough = d.lengthSquared() < squaredViewDistance or isEqual(0, d.lengthSquared());

    double dotProduct = mDirection.dot(d.normalised());
    bool isInViewRange = dotProduct < std::cos(getViewRange() + 0.001);

    return isCloseEnough and isInViewRange;
}
